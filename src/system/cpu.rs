...
impl CPU {
    ...
    pub fn handle_interrupts(&mut self, mem: &mut Memory) {
        ...
        // Check if IRQs are disabled in CPSR
        ...
        // 3. Save return address (PC+4 for ARM, PC+2 for THUMB)
        let return_addr = self.r[REGISTER_PC as usize] + self.instruction_len_in_bytes();
        self.set_r(REGISTER_LR, return_addr);

        // 4. Switch to ARM mode
        self.set_thumb_state(false);

        // 5. Jump to IRQ vector at 0x18
