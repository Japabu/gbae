...
pub fn decode_branch_exchange_thumb(instruction: u16, next_instruction: u16) -> Box<dyn super::DecodedInstruction> {
    let l = get_bit16(instruction, 7);
    if l {
        // BLX (2) not implemented yet - return unknown instruction instead of panicking
        return super::lut::UnknownInstruction::decode_thumb(instruction, next_instruction);
    }
    Box::new(Opcode::BRegister {
        l,
        x: true,
...
pub fn decode_bl_thumb(instruction: u16, next_instruction: u16) -> Box<dyn super::DecodedInstruction> {
    // Check if this is a valid BL Thumb instruction pair
    if get_bits16(instruction, 11, 2) != 0b10 || get_bits16(next_instruction, 11, 2) != 0b11 {
        // Invalid instruction pattern, return unknown
        return super::lut::UnknownInstruction::decode_thumb(instruction, next_instruction);
    }
