...
pub enum Condition {
    ...
    LE, // Less or Equal
    AL, // Always
    NV, // Never/Invalid (reserved/undefined condition code)
}
...
impl Condition {
    pub const fn parse(cond: u8) -> Condition {
        match cond {
            ...
            0b1101 => Condition::LE,
            0b1110 => Condition::AL,
            _ => Condition::NV, // Invalid/reserved condition code - never executes
        }
    }
    ...
    pub fn check(&self, cpu: &CPU) -> bool {
        match self {
            ...
            Condition::AL => true,
            Condition::NV => false, // Never - invalid/reserved condition
        }
    }
}
