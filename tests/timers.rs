...
fn timer_counts_every_cycle_without_prescaler() {
    ...
    let elapsed = steps(&mut gba, 100);
    assert!(elapsed >= 100);
    assert_eq!(gba.mem.read_u16(TM0CNT_L), elapsed as u16);
}
...
fn timer_counts_with_prescaler_64() {
    ...
    let elapsed = steps(&mut gba, 64);
    assert!(elapsed >= 128);
    assert_eq!(gba.mem.read_u16(TM0CNT_L), (elapsed / 64) as u16);
}
...
fn timer_reloads_and_raises_irq_on_overflow() {
    ...
    let mut elapsed = 0;
    while elapsed + 3 < 16 {
        elapsed += steps(&mut gba, 1);
        assert_eq!(gba.mem.read_u16(TM0CNT_L), 0xFFF0 + elapsed as u16);
        assert_eq!(gba.mem.read_u16(IF) & TIMER0_IRQ, 0);
    }
    elapsed += steps(&mut gba, 1);
    assert!(elapsed >= 16);
    assert_eq!(gba.mem.read_u16(TM0CNT_L), 0xFFF0 + (elapsed - 16) as u16);
...
fn count_up_timer_counts_overflows_of_previous_timer() {
    ...
    gba.mem.write_u16(TM0CNT_H, ENABLE);
    let elapsed = steps(&mut gba, 4);
    assert_eq!(gba.mem.read_u16(TM1CNT_L), elapsed as u16);
}
